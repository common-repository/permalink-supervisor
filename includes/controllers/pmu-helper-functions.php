<?php
class PMU_Helper_Functions extends PMU_Class {
	...
	public function init() {
		...
		add_filter( 'permalink_updator_filter_default_post_uri', array($this, 'replace_empty_placeholder_tags'), 10, 5);
		add_filter( 'permalink_updator_filter_default_term_uri', array($this, 'replace_empty_placeholder_tags'), 10, 5);

		// Clear the final default URIs
		add_filter( 'permalink_updator_filter_default_term_uri', array($this, 'clear_single_uri'), 20);
		add_filter( 'permalink_updator_filter_default_post_uri', array($this, 'clear_single_uri'), 20);
	...
	static function get_primary_term($post_id, $taxonomy, $slug_only = true) {
		...
		$primary_term_enabled = apply_filters('permalink_updator_primary_term', true);
	...
	static function get_term_full_slug($term, $terms, $mode = false, $native_uri = false) {
		...
		if($mode == 1) {
			$term_slug = $puUris["tax-{$term->term_id}"];
		}
		...
		// D. Flat/non-hierarchical taxonomy base - get primary term (if set) or first term
			...
			$term_slug = ($native_uri) ? $term->slug : PMU_Helper_Functions::force_custom_slugs($term->slug, $term);
		}
	...
	static function get_disabled_post_types($include_user_excluded = true) {
		global $wp_post_types, $puUpdatorOptions;
		...
		if($include_user_excluded) {
			$disabled_post_types = (!empty($puUpdatorOptions['general']['partial_disable']['post_types'])) ? array_merge((array) $puUpdatorOptions['general']['partial_disable']['post_types'], $disabled_post_types) : $disabled_post_types;
		}

		return apply_filters('permalink_updator_disabled_post_types', $disabled_post_types);
	}

	static function get_disabled_taxonomies($include_user_excluded = true) {
		global $wp_taxonomies, $puUpdatorOptions;
		...
		if($include_user_excluded) {
			$disabled_taxonomies = (!empty($puUpdatorOptions['general']['partial_disable']['taxonomies'])) ? array_merge((array) $puUpdatorOptions['general']['partial_disable']['taxonomies'], $disabled_taxonomies) : $disabled_taxonomies;
		}

		return apply_filters('permalink_updator_disabled_taxonomies', $disabled_taxonomies);
	}
	...
	/**
	 * Get all post types supported by Permalink Supervisor
	 */
	...
	/**
	 * Get all taxonomies supported by Permalink Supervisor
	 */
	...
	/**
	 * Get all post statuses supported by Permalink Supervisor
	 */
	static function get_post_statuses() {
		$post_statuses = get_post_statuses();

		return apply_filters('permalink_updator_post_statuses', $post_statuses);
	}
	...
	*/
	...
	static function get_endpoints() {
		...
		return apply_filters("permalink_updator_endpoints", str_replace("/", "\/", $endpoints));
	}
	...
	*/
	...
	public static function get_permalink_base($element = null) {
		return apply_filters('permalink_updator_filter_permalink_base', trim(get_option('home'), "/"), $element);
	}
	...
	static function is_front_page($page_id) {
		...
		return apply_filters('permalink_updator_is_front_page', $bool, $page_id, $front_page_id);
	}
	...
	public static function sanitize_title($str, $keep_percent_sign = false, $force_lowercase = null, $sanitize_slugs = null) {
		...
		$force_lowercase = (!is_null($force_lowercase)) ? $force_lowercase : apply_filters('permalink_updator_force_lowercase_uris', true);

		if(is_null($sanitize_slugs)) {
			$sanitize_slugs = (!empty($puUpdatorOptions['general']['disable_slug_sanitization'])) ? false : true;
		}
		...
		// Remove accents
		$clean = (empty($puUpdatorOptions['general']['keep_accents'])) ? remove_accents($clean) : $clean;

		$percent_sign = ($keep_percent_sign) ? "\%" : "";
		//$sanitize_regex = apply_filters("permalink_updator_sanitize_regex", "/[^\p{Thai}\p{Greek}\p{Hebrew}\p{Arabic}\p{Cyrillic}a-zA-Z0-9{$percent_sign}\/_\.|+, -]/u", $percent_sign);
		$sanitize_regex = apply_filters("permalink_updator_sanitize_regex", "/[^\p{Xan}a-zA-Z0-9{$percent_sign}\/_\.|+, -]/ui", $percent_sign);
	...
	public static function replace_empty_placeholder_tags($default_uri, $native_slug = "", $element = "", $slug = "", $native_uri = "") {
		...
		$empty_tag_replacement = apply_filters('permalink_updator_empty_tag_replacement', null, $element);
	...
	public static function force_custom_slugs($slug, $object, $flat = false) {
		...
		$force_custom_slugs = (!empty($puUpdatorOptions['general']['force_custom_slugs'])) ? $puUpdatorOptions['general']['force_custom_slugs'] : false;
		$force_custom_slugs = apply_filters('permalink_updator_force_custom_slugs', $force_custom_slugs, $slug, $object);
		...
		if($force_custom_slugs) {
			...
			// B. Custom slug (custom permalink)
				...
				$new_slug = (!empty($puUris[$object_id])) ? basename($puUris[$object_id]) : '';
			}
	...
	public static function get_all_duplicates($include_custom_uris = true) {
		global $puUris, $puRedirects, $puUpdatorOptions, $wpdb;

		// Make sure that both variables are arrays
		$all_uris = ($include_custom_uris && is_array($puUris)) ? $puUris : array();
		$puRedirects = (is_array($puRedirects)) ? $puRedirects : array();

		// Convert redirects list, so it can be merged with $puUris
		foreach($puRedirects as $element_id => $redirects) {
	...
	public static function is_uri_duplicated($uri, $element_id) {
		global $puUris;
 		...
 		if(in_array($uri, $all_uris)) {
			...
			$this_uri_lang = PMU_Language_Plugins::get_language_code($element_id);

			if($this_uri_lang) {
				foreach($all_duplicates as $key => $duplicated_id) {
					$duplicated_uri_lang = PMU_Language_Plugins::get_language_code($duplicated_id);
	...
	public function reload_globals_in_network($new_blog_id) {
	  global $puUris, $puRedirects, $puExternalRedirects;

		if(function_exists('get_blog_option')) {
		  $puUris = get_blog_option($new_blog_id, 'permalink-updator-uris');
		  $puRedirects = get_blog_option($new_blog_id, 'permalink-updator-redirects');
		  $puExternalRedirects = get_blog_option($new_blog_id, 'permalink-updator-external-redirects');
		}
	}
