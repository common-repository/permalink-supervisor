<?php
class PMU_Debug_Functions extends PMU_Class {
	...
	public function debug_data() {
		add_filter('permalink_updator_filter_query', array($this, 'debug_query'), 9, 5);
		add_filter('permalink_updator_filter_redirect', array($this, 'debug_redirect'), 9, 3);
	...
	public function debug_query($query, $old_query = null, $uri_parts = null, $pm_query = null, $content_type = null) {
		...
		if(isset($_REQUEST['debug_url'])) {
			...
			if(class_exists('PMU_Pro_Functions')) {
				$license_key = $permalink_updator->functions['pro-functions']->get_license_key();
			...
			// Plugin version
			$debug_info['version'] = PMU_PLVERSION;
	...
	/**
	 * 2. Used in PMU_Core_Functions::new_uri_redirect_and_404();
	 */
	...
	public function debug_custom_redirects() {
		global $permalink_updator, $puUris, $puRedirects;

		if(isset($_GET['debug_custom_redirects']) && current_user_can('manage_options')) {
			if(!empty($puUris)) {
				$uri_post_class = $permalink_updator->functions['uri-functions-post'];
				$uri_tax_class = $permalink_updator->functions['uri-functions-tax'];
				...
				// Native redirects
				foreach($puUris as $element_id => $uri) {
			...
			if($puRedirects) {
				foreach($puRedirects as $element_id => $redirects) {
					if(empty($puUris[$element_id])) { continue; }
					$custom_permalink = user_trailingslashit($home_url . "/" . $puUris[$element_id]);
