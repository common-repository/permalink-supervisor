<?php
class PMU_URI_Functions_Tax extends PMU_Class {
	...
	public function init() {
		...
		foreach($all_taxonomies as $tax => $label) {
			...
			if(PMU_Helper_Functions::is_disabled($tax, 'taxonomy')) { continue; }
	...
	function custom_tax_permalinks($permalink, $term) {
		...
		if(empty($term->term_id) || (!empty($term->taxonomy) && PMU_Helper_Functions::is_disabled($term->taxonomy, 'taxonomy'))) { return $permalink; }
		...
		if(isset($puUris["tax-{$term_id}"])) {
			// Start with homepage URL
			$permalink = PMU_Helper_Functions::get_permalink_base($term);

			// Encode URI?
			if(!empty($puUpdatorOptions['general']['decode_uris'])) {
				$permalink .= rawurldecode("/{$puUris["tax-{$term_id}"]}");
			} else {
				$permalink .= PMU_Helper_Functions::encode_uri("/{$puUris["tax-{$term_id}"]}");
			}
		} else if(!empty($puUpdatorOptions['general']['decode_uris'])) {
			$permalink = rawurldecode($permalink);
		}

		return apply_filters('permalink_updator_filter_final_term_permalink', $permalink, $term, $old_permalink);
	}
	...
	public static function get_term_uri($term_id, $native_uri = false) {
		...
		$final_uri = (!empty($puUris["tax-{$term_id}"])) ? $puUris["tax-{$term_id}"] : self::get_default_term_uri($term->term_id, $native_uri);
		return $final_uri;
	}

	
	public static function get_default_term_uri($term, $native_uri = false, $check_if_disabled = false) {
		global $puUpdatorOptions, $puUris, $puPermastructs, $wp_rewrite, $wp_taxonomies;
		...
		if($check_if_disabled && PMU_Helper_Functions::is_disabled($taxonomy, 'taxonomy')) { return ''; }

		$native_permastructure = PMU_Helper_Functions::get_default_permastruct($taxonomy_name, true);
		if($native_uri || empty($puPermastructs['taxonomies'][$taxonomy_name])) {
			$permastructure = $native_permastructure;
		} else {
			$permastructure = apply_filters('permalink_updator_filter_permastructure', $puPermastructs['taxonomies'][$taxonomy_name], $term);
		}

		
		$default_base = (!empty($permastructure)) ? trim($permastructure, '/') : "";
		if(empty($default_base) && !isset($puPermastructs['taxonomies'][$taxonomy_name])) {
			...
			$term_slug 			= PMU_Helper_Functions::remove_slashes($term_slug);
			$custom_slug 		= $full_custom_slug = PMU_Helper_Functions::force_custom_slugs($term_slug, $term);
			...
			if(is_taxonomy_hierarchical($taxonomy_name)) {
				...
				foreach($ancestors as $ancestor) {
					...
					$full_custom_slug = PMU_Helper_Functions::force_custom_slugs($ancestor_term->slug, $ancestor_term) . '/' . $full_custom_slug;
				}
			}

			// Allow filter the default slug (only custom permalinks)
			if(!$native_uri) {
				$full_slug = apply_filters('permalink_updator_filter_default_term_slug', $full_custom_slug, $term, $term->name);
			} else {
			...
			$taxonomy_name_slug = apply_filters('permalink_updator_filter_taxonomy_slug', $taxonomy_name_slug, $term, $taxonomy_name);
			...
			$do_not_append_slug = (!empty($puUpdatorOptions['permastructure-settings']['do_not_append_slug']['taxonomies'][$taxonomy_name])) ? true : false;
			$do_not_append_slug = apply_filters("permalink_updator_do_not_append_slug", $do_not_append_slug, $taxonomy, $term);
		...
		return apply_filters('permalink_updator_filter_default_term_uri', $default_uri, $term->slug, $term, $term_slug, $native_uri);
	}
	...
	public static function get_items() {
		...
		$taxonomy_names_array = sanitize_text_field($_POST['taxonomies']) ? sanitize_text_field($_POST['taxonomies']) : '';
		...
		if(!empty($_POST['ids'])) {
			...
			$ids = esc_sql(preg_replace('/\s*/m', '', sanitize_text_field($_POST['ids'])));
	...
	public static function find_and_replace($chunk = null, $mode = '', $old_string = '', $new_string = '') {
		...
		if($terms_to_update && empty($errors)) {
			foreach ($terms_to_update as $row) {
				...
				$old_uri = (isset($puUris[$term_permalink_id])) ? $puUris[$term_permalink_id] : $native_uri;
				...
				if(($old_uri != $new_uri) || ($old_term_name != $new_term_name)) {
					$puUris[$term_permalink_id] = $new_uri;
				...
				do_action('permalink_updator_updated_term_uri', $row['term_id'], $new_uri, $old_uri, $native_uri, $default_uri);
			}
	...
	static function regenerate_all_permalinks($chunk = null, $mode = '') {
		...
		if($terms_to_update && empty($errors)) {
			foreach ($terms_to_update as $row) {
				...
				$default_uri = self::get_default_term_uri($this_term);
				$old_term_name = $row['slug'];
				$old_uri = (isset($puUris[$term_permalink_id])) ? $puUris[$term_permalink_id] : '';
				$correct_slug = ($mode == 'slugs') ? sanitize_title($row['name']) : PMU_Helper_Functions::sanitize_title($row['name']);
	...
	static public function update_all_permalinks() {
		...
		if (!empty($new_uris)) {
			foreach($new_uris as $id => $new_uri) {
				...
				do_action('permalink_updator_updated_term_uri', $term_id, $new_uri, $old_uri, $native_uri, $default_uri);
			}

			// Filter array before saving & append the global
			$old_uris = $puUris = array_filter($old_uris);
			update_option('permalink-updator-uris', $old_uris);
	...
	public function edit_uri_box($term = '') {
		...
		if(!empty($term->taxonomy)) {
			$show_uri_editor = apply_filters("permalink_updator_hide_uri_editor_term_{$term->taxonomy}", true);

			if(!$show_uri_editor) { return; }
		}

		$label = __("Custom URI", "permalink-updator");
		$description = __("Clear/leave the field empty to use the default permalink.", "permalink-updator");
		...
		// B. Edit term
			...
			$custom_uri = (!empty($puUris["tax-{$term->term_id}"])) ? $puUris["tax-{$term->term_id}"] : true;

			$html = "<tr id=\"permalink-updator\" class=\"form-field permalink-updator-edit-term permalink-updator\">";
			$html .= "<th scope=\"row\" valign=\"top\"><label for=\"custom_uri\">{$label}</label></th>";
			$html .= "<td><div>";
			$html .= PMU_Admin_Functions::display_uri_box($term);
			$html .= "</div></td>";
			$html .= "</tr>";
		}
	...
	function quick_edit_column($columns) {
		return array_merge($columns, array('permalink-updator-col' => __( 'Current URI', 'permalink-updator')));
	}

	function quick_edit_column_content($content, $column_name, $term_id) {
		global $puUris;

		if($column_name != "permalink-updator-col") { return $content; }
		return (!empty($puUris["tax-{$term_id}"])) ? self::get_term_uri($term_id) : self::get_term_uri($term_id, true);
	}
	...
	function update_term_uri($term_id, $taxonomy) {
		global $puUris, $puUpdatorOptions, $wp_current_filter, $current_screen;
		...
		} else if(isset($_POST['custom_uri']) && (!isset($_POST['permalink-updator-nonce']) || !wp_verify_nonce($_POST['permalink-updator-nonce'], 'permalink-updator-edit-uri-box'))) { return; }
		...
		// Check if taxonomy is allowed
		if(PMU_Helper_Functions::is_disabled($this_term->taxonomy, 'taxonomy')) { return; }

		// Stop the hook (if needed)
		$allow_update_term = apply_filters("permalink_updator_update_term_uri_{$this_term->taxonomy}", true);
		...
		if(!empty($_POST['custom_uri']) && empty($force_default_uri) && empty($_POST['post_ID']) && $auto_update_uri !== 1) {
			$new_uri = PMU_Helper_Functions::sanitize_title($_POST['custom_uri']);
		} else {
			$new_uri = $default_uri;
		}
