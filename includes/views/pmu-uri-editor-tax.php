<?php
class PMU_Tax_Uri_Editor_Table extends WP_List_Table {
	...
	*/
	public function display_admin_section() {
		...
		$output .= PMU_Admin_Functions::section_type_field('taxonomies');
	...
	*/
	public function get_columns() {
		return apply_filters('permalink_updator_uri_editor_columns', array(
			...
			'item_title'		=> __('Term title', 'permalink-updator'),
			'item_uri'	=> __('Full URI & Permalink', 'permalink-updator'),
			'count'	=> __('Count', 'permalink-updator'),
		));
	}
	...
	*/
	public function column_default($item, $column_name) {
		...
		$uri = PMU_URI_Functions_Tax::get_term_uri($item['term_id'], true);
		...
		$output = apply_filters('permalink_updator_uri_editor_column_content', '', $column_name, $term);
		...
		switch($column_name) {
			...
			case 'item_title':
				...
				$output .= sprintf("<span><strong>%s:</strong> %s</span>", __("Slug", "permalink-updator"), urldecode($item['slug']));
				$output .= apply_filters('permalink_updator_uri_editor_extra_info', '', $column_name, $term);
				$output .= '</div>';

				$output .= '<div class="row-actions">';
				$output .= sprintf("<span class=\"edit\"><a href=\"%s\" title=\"%s\">%s</a> | </span>", esc_url(get_edit_tag_link($item['term_id'], $item['taxonomy'])), __('Edit', 'permalink-updator'), __('Edit', 'permalink-updator'));
				$output .= '<span class="view"><a target="_blank" href="' . $permalink . '" title="' . __('View', 'permalink-updator') . ' ' . $item[ 'name' ] . '" rel="permalink">' . __('View', 'permalink-updator') . '</a> | </span>';
			...
			case 'item_uri':
				...
				$output .= PMU_Admin_Functions::generate_option_field("uri[tax-{$item['term_id']}]", $field_args_base);
	...
	*/
	public function prepare_items() {
		global $wpdb, $puUpdatorOptions, $puUris, $active_subsection, $current_admin_tax;
		...
		$taxonomies_array = PMU_Helper_Functions::get_taxonomies_array();
		...
		$orderby = (isset($_REQUEST['orderby'])) ? sanitize_text_field($_REQUEST['orderby']) : 't.term_id';
		...
		if($search_query) {
			...
			$found = PMU_Helper_Functions::search_uri($search_query, 'taxonomies');
			if($found) {
		...
		// Get items
		$sql_query = apply_filters('permalink_updator_filter_uri_editor_query', $sql_query, $this, $sql_parts, $is_taxonomy = true);
