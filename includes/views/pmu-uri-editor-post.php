<?php
class PMU_URI_Editor_Post extends WP_List_Table {

	public $displayed_post_types, $displayed_post_statuses;

	public function __construct() {
		global $status, $page, $puUpdatorOptions, $active_subsection;
		...
		$this->displayed_post_statuses = (isset($puUpdatorOptions['screen-options']['post_statuses'])) ? "'" . implode("', '", $puUpdatorOptions['screen-options']['post_statuses']) . "'" : "'no-post-status'";
		$this->displayed_post_types = ($active_subsection && $active_subsection == 'all') ? "'" . implode("', '", $puUpdatorOptions['screen-options']['post_types']) . "'" : "'{$active_subsection}'";
	}
	...
	*/
	public function display_admin_section() {
		...
		$output .= wp_nonce_field('permalink-updator', 'uri_editor', true, true);
		$output .= PMU_Admin_Functions::generate_option_field('pm_session_id', array('value' => uniqid(), 'type' => 'hidden'));
	...
	*/
	public function get_columns() {
		return apply_filters('permalink_updator_uri_editor_columns', array(
			'item_title'		=> __('Post title', 'permalink-updator'),
			'item_uri'	=> __('Full URI & Permalink', 'permalink-updator')
		));
	}
	...
	*/
	public function column_default( $item, $column_name ) {
		...
		$uri = PMU_URI_Functions_Post::get_post_uri($item['ID'], true);
		$uri = (!empty($puUpdatorOptions['general']['decode_uris'])) ? urldecode($uri) : $uri;
		...
		$post_statuses_array['inherit'] = __('Inherit (Attachment)', 'permalink-updator');

		$output = apply_filters('permalink_updator_uri_editor_column_content', '', $column_name, get_post($item['ID']));
		if(!empty($output)) { return $output; }

		switch( $column_name ) {
			case 'item_uri':
				$output = '<div class="custom_uri_container">';
				$output .= PMU_Admin_Functions::generate_option_field("uri[{$item['ID']}]", $field_args_base);
			...
			case 'item_title':
				...
				$output .= sprintf("<span><strong>%s:</strong> %s</span>", __("Slug", "permalink-updator"), urldecode($item['post_name']));
				$output .= sprintf(" | <span><strong>%s:</strong> {$post_statuses_array[$item["post_status"]]}</span>", __("Post status", "permalink-updator"));
				$output .= apply_filters('permalink_updator_uri_editor_extra_info', '', $column_name, get_post($item['ID']));
				$output .= '</div>';

				$output .= '<div class="row-actions">';
				$output .= sprintf("<span class=\"edit\"><a href=\"%s/wp-admin/post.php?post={$item['ID']}&amp;action=edit\" title=\"%s\">%s</a> | </span>", get_option('home'), __('Edit', 'permalink-updator'), __('Edit', 'permalink-updator'));
				$output .= '<span class="view"><a target="_blank" href="' . $permalink . '" title="' . __('View', 'permalink-updator') . ' ' . $item[ 'post_title' ] . '" rel="permalink">' . __('View', 'permalink-updator') . '</a> | </span>';
	...
	*/
	function extra_tablenav($which) {
		...
		$button_top = __( 'Save Changes', 'permalink-updator' );
		$button_bottom = __( 'Save Changes', 'permalink-updator' );
    ...
    if ($which == "top") {
			...
			$html .= $this->search_box(__('Search', 'permalink-updator'), 'search-input');
			$html .= '</div>';
			...
			if($months) {
				...
				$html .= sprintf("<option value=\"\">%s</option>", __("All dates", "permalink-updator"));
				...
				$html .= "</select>";
				$html .= get_submit_button(__("Filter", "permalink-updator"), 'button', false, false, array('id' => 'months-filter-button', 'name' => 'months-filter-button'));
				$html .= "</div>";
			}
    }
		$html .= "</div>";
	...
	*/
    ...
    $output = "<p class=\"search-box\">";
		...
		$output .= PMU_Admin_Functions::generate_option_field('s', array('value' => $search_query, 'type' => 'search'));
	...
	*/
	public function prepare_items() {
		...
		// Get query variables
		$per_page = $puUpdatorOptions['screen-options']['per_page'];
		...
		if($search_query) {
			...
			$found = PMU_Helper_Functions::search_uri($search_query, 'posts');
			if($found) {
		...
		// Get items
		$sql_query = apply_filters('permalink_updator_filter_uri_editor_query', $sql_query, $this, $sql_parts, $is_taxonomy = false);
